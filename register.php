...
<body> 
	...
	<!--start-account-->
	<div class="account">
		<div class="container" style="border:1px solid RGB(0, 0, 0);padding:20px 15px 20px 15px;"> 
			<div class="account-bottom">
				...
				<div class="col-md-6 account-left">
					...
					<div class="address reg">
						...
						<input type="text" style="border-radius:5px;" name="customer_first_name" placeholder="John" value="<?php echo isset($_POST["customer_first_name"]) ? $_POST["customer_first_name"] : ''; ?>"required>


						<span>Last Name </span>
						<input type="text" style="border-radius:5px;" name="customer_last_name" placeholder="William" value="<?php echo isset($_POST["customer_last_name"]) ? $_POST["customer_last_name"] : ''; ?>"required>


						<span>Email Address</span> 
						<input type="email" name="customer_email" style="border-radius:5px;border: 1px solid #242424;outline: none;width: 100%;font-size: 14px;padding: 10px 10px;font-family: 'Lato', sans-serif;margin: 10px 0px 10px 0px;" placeholder="JohnWilliam420@gmail.com" value="<?php echo isset($_POST["customer_email"]) ? $_POST["customer_email"] : ''; ?>"required>
						<span style="color:red;"><?php echo $email_check; ?></span>

						<span>Handphone No.</span> 
						<input type="text" placeholder="e.g  0123456789" style="border-radius:5px;" name="customer_phone_num" pattern="0.+" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" maxlength="11" value="<?php echo isset($_POST["customer_phone_num"]) ? $_POST["customer_phone_num"] : ''; ?>" required>
						...
						<span>Password</span>
						<input type="password" style="border-radius:5px;" name="customer_password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{14,}" required>
						...
						<input type="password" style="border-radius:5px;" name="confirm_password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{14,}" required>
				...
				<div class="col-md-6 account-right">
					...
					<div class="address reg">
						...
						<input type="text" style="border-radius:5px;" name="address1" placeholder="123, Jalan Example 1/20" value="<?php echo isset($_POST["address1"]) ? $_POST["address1"] : ''; ?>" required>


						<span>Addresss Line 2</span>
						<input type="text" style="border-radius:5px;" name="address2" placeholder="Taman Example" value="<?php echo isset($_POST["address2"]) ? $_POST["address2"] : ''; ?>" required>


						<span>Postcode</span>
						<input type="text" maxlength="5" style="border-radius:5px;" placeholder="81200" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="postcode" value="<?php echo isset($_POST["postcode"]) ? $_POST["postcode"] : ''; ?>" required>
						<span style="color:black;">*Only number available</span>
					
						<span style="color:red;"><?php echo $postcode_check; ?></span>

						<span>City</span>
						<input type="text" style="border-radius:5px;"  name="city" placeholder="Kota Kinabalu" value="<?php echo isset($_POST["city"]) ? $_POST["city"] : ''; ?>" required>

						<span style="margin-top:25px;">State</span>
						<select class="form-control" id="state" style="border-radius:5px;" name="state" required>
